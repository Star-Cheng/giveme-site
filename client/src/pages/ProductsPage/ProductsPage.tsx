  ...
  return (
    <div className="pt-28">
      ...
      <section className="pb-16">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-xs text-[#2563eb] font-medium tracking-wider uppercase mb-4">产品矩阵</p>
              <h1 className="text-4xl md:text-5xl font-bold text-[#0f172a] mb-6 leading-tight">
                智能超快激光
                <br />
                <span className="text-[#2563eb]">从基础到极致</span>
              </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {laserSeries.map((s, i) => (
              <div key={s.name} className={`animate-on-scroll delay-${(i + 1) * 100} bg-white rounded-2xl overflow-hidden border border-[#f1f5f9]`}>
                ...
                <div className="p-6">
                  <span className="text-xs text-[#2563eb] font-medium tracking-wider uppercase">{s.tier}</span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {rockCore.map((rc) => (
              <div key={rc.name} className="p-5 rounded-2xl bg-[#f8fafc] border border-[#f1f5f9]">
                ...
                <p className="text-[10px] text-[#2563eb] font-medium tracking-wider uppercase mb-2">{rc.english}</p>
              ...
              <tbody>
                {[
                  ...
                  <tr key={r[0]} className="border-b border-[#f1f5f9] last:border-0">
                    ...
                    <td className="p-4 text-[#2563eb] font-medium">{r[1]}</td>
      ...
      <section className="pb-20 text-center">
        <Link to="/contact" className="inline-flex items-center gap-2 px-8 py-3.5 bg-[#0f172a] text-white text-sm font-medium rounded-full hover:bg-[#2563eb] transition-colors">
          预约技术交流
